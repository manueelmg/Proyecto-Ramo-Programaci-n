...
namespace MineriaDatos
{
    public partial class ManejoDatos : Form
    {
        ...
        private void btnCargaCSV_Click(object sender, EventArgs e)
        {
            ...
            string pathFile = @"C:\Users\manue\Desktop\BaseMovil.csv";  //Aca colocar la ruta de donde dejaron el archivo CSV en SU PC
        ...
        private string ConstructorFiltroFechas(DataTable dataTable, string selectedColumn)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(startDateInput) && DateTime.TryParseExact(startDateInput, "dd-MM-yyyy", System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.None, out DateTime startDate))
                {
                    ...
                    {
                        // Si no se ingresa una fecha final, filtrar por la fecha exacta
                        dateFilter = $"[{selectedColumn}] = #{startDate:MM/dd/yyyy}#";
                    }

                    else if (DateTime.TryParseExact(endDateInput, "dd-MM-yyyy", System.Globalization.CultureInfo.InvariantCulture, System.Globalization.DateTimeStyles.None, out DateTime endDate))
                    {
                        // Si hay una fecha final válida, filtrar por rango
                        dateFilter = $"[{selectedColumn}] >= #{startDate:MM/dd/yyyy}# AND [{selectedColumn}] <= #{endDate:MM/dd/yyyy}#";
                    }
                    else
                    {
                        MessageBox.Show("La fecha final no es válida. Por favor, inténtelo nuevamente.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return null;
                    }
                }
                else
        ...
        private void BorrarFiltros_Click(object sender, EventArgs e)
        {
            if (dtgProducts.DataSource is DataTable dataTable)
            {
                ...
                MessageBox.Show("Filtros eliminados. La tabla se ha restaurado a su estado original.", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
        ...
        private void EnviarDatos_Click(object sender, EventArgs e)
        {
            if (dtgProducts.DataSource is DataTable dataTable)
            {
                // Crear una instancia de GraficosDatos y pasar el DataTable
                GraficosDatos graficosDatosForm = new GraficosDatos(dataTable);

                // Mostrar el formulario GraficosDatos
                graficosDatosForm.Show();
            }
            else
            {
                MessageBox.Show("No hay datos cargados para enviar.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
    }
}
